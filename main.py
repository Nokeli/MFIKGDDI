...
from models import MFIKDDI
from models import KGCN
from models import DDKG_OD
from config import ModelConfig, PROCESSED_DATA_DIR,  ENTITY_VOCAB_TEMPLATE, \
    ...
    DRUG_VOCAB_TEMPLATE,SMILE_HASH,DRUG_SMILE_TEMPLATE,MOLE_EMBEDDING,SELF_EMBEDDING
...
def train(train_d,dev_d,test_d,kfold,dataset, neighbor_sample_size, embed_dim, n_depth, l2_weight, lr, optimizer_type,
    ...
    config.mole_embedding = np.load(format_filename(PROCESSED_DATA_DIR, MOLE_EMBEDDING),allow_pickle=True)

    config.self_embedding = np.load(format_filename(PROCESSED_DATA_DIR, SELF_EMBEDDING),allow_pickle=True)
